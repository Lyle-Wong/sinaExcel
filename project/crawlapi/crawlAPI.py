...
class Crawl:
	...
	def __init__(self):
		self.day_url = domain + self.day_url + "?v=" + str(time.time()*100000)
		self.week_url = domain + self.week_url + "?v=" + str(time.time()*100000)
		self.month_url = domain + self.month_url + "?v=" + str(time.time()*100000)
	...
	def ex_rate_input(self):
		...
		excelUtil.excel_write(excel_file_path, "（美联储）", "C", "4", yesterday_close)
		excelUtil.excel_write(excel_file_path, "（美联储）", "C", "6", seurusd_value[3])
		excelUtil.excel_write(excel_file_path, "（美联储）", "C", "10", sgbpusd_value[3])
		excelUtil.excel_write(excel_file_path, "（美联储）", "C", "8", susdjpy_value[3])
	...
	def bdi(self):
		...
		for num in range(data_count):
			...
			excelUtil.excel_write(excel_file_path, index_bdi, "C", str(2*num+3) , data_week[num]['index'])
			excelUtil.excel_write(excel_file_path, index_bdi, "D", str(2*num+3) , data_week[num]['date'])

		excelUtil.excel_write(excel_file_path, index_bdi, "C", "2", data_month[-6]['index'])
		excelUtil.excel_write(excel_file_path, index_bdi, "D", "2" , data_month[-6]['date'])
